...
既存 Issue でカバーされていなかった作業として、#19〜#32 を新規に作成した。
...
| #31 | feat: Title / Result 画面の実装 | FE②UI | M1 |
| #32 | feat: /health でDB疎通も確認するか検討・対応する | BE | M2 |
...
対象: #19, #20, #21, #22, #23, #24, #25, #26, #31

### M2: 公開＋スコア永続化

**完了条件:** HTTPS の公開URLで遊べて、スコアが DB に保存され、ランキング画面に出る。

対象: #13, #14, #16, #17, #27, #28, #29, #32
...
4. **#31 Title / Result 画面の実装** — 現状スタブ。プレイヤー名入力（`player_name` が必要）、リザルト表示
...
3. **#32 `/health` の強化**（小）— デプロイ後のヘルスチェックで使うので DB 疎通も見るか判断する
4. **#12 対戦用サーバー実装**（M3）
